...
\]
which is the statement that the uniform norm on an $n$-dimensional space of functions is
dominated by the $L_2$ norm of that measure, with the constant $\sqrt{n + \varepsilon}$.
The answer of Kiefer and Wolfowitz is that a measure whose Gram matrix has an almost
maximal determinant does this.
...
\begin{proof}
\leanok
It suffices to find points $x_1, \dots, x_N$ such that a coefficient vector annihilated
by all of them is zero, for then the uniform distribution on those points has
$c^* G c > 0$ for every $c \neq 0$ by \cref{lem:design-quadform}. Such points are found
by descending induction on dimension: if some nonzero vector of the subspace still
surviving is annihilated by the points chosen so far, the hypothesis supplies a point at
which it does not vanish, and adding that point lowers the dimension of the surviving
subspace by at least one.
\end{proof}
...
\leanok
Let $A \in \mathbb{C}^{\iota \times \iota}$ be invertible, $\beta \neq 0$ a scalar,
$\alpha$ a scalar and $u \in \mathbb{C}^{\iota}$. Then
\[
  \det(\beta A + \alpha \, u u^*)
    = \beta^{\,n-1} \det A \, \bigl( \beta + \alpha \, u^* A^{-1} u \bigr).
\]
\end{lemma}
...
Let $D$ carry a measurable structure and let the functions $a_k$ be measurable in
addition. Then the measure $\varrho = \sum_k w_k \delta_{x_k}$ of
\cref{thm:uniform-discretization} is a probability measure whose Gram matrix $G(a)$ in
the sense of \cref{def:gram} is positive definite and equal to the Gram matrix of the
design, and
\[
...
formalised; nothing needs it, because the sparsification theorem accepts any finite, and
indeed any countable, family of points.
\end{remark}
...
\]
for every $z \in \mathbb{C}^{\iota}$: the first because its left-hand side is
$\Tr(G^{-1} G)$, the second because it is $G G^{-1} z = z$ written out. With
$c_k = w_k \, a(x_k)^* G^{-1} a(x_k)$ the two say that the $c_k$ are nonnegative, sum to
$n$, and decompose the identity over the points of the design in the inner product
$\langle u, z \rangle = u^* G^{-1} z$ that the ellipsoid of $G$ defines. This is John's
decomposition of the identity, the condition dual to the bound
$a(y)^* G^{-1} a(y) \leq n + \varepsilon$ of \cref{prop:det-max}. Under
\cref{thm:uniform-discretization-compact} that bound holds with $\varepsilon = 0$, and
together with the average $n$ it forces $a(x_k)^* G^{-1} a(x_k) = n$ at every point of
the design, so that the points are contact points; in general the bound carries its
$\varepsilon$, which is John position up to $\varepsilon$. For an arbitrary convex body
the theorem is proved in the companion project
