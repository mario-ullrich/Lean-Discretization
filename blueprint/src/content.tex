...
The last chapter stands apart from the rest. It answers the question which measure to
discretize in the first place: on an $n$-dimensional space of functions, a measure
maximising the determinant of the Gram matrix already controls the uniform norm by the
$L_2$ norm.
...
\begin{lemma}[Determinant of a rank-one mixture]
\label{lem:det-rank-one}
\lean{Matrix.det_smul_add_smul_vecMulVec}
\leanok
Let $A$ be an invertible $m \times m$ matrix, $\beta \neq 0$ a scalar, $\alpha$ a scalar
and $u$ a vector. Then
\[
  \det(\beta A + \alpha \, u u^*)
    = \beta^{\,m-1} \det A \, \bigl( \beta + \alpha \, u^* A^{-1} u \bigr).
\]
\end{lemma}

\begin{proof}
\leanok
Factor $\beta A$ out and apply the matrix determinant lemma
$\det(A + v w^*) = \det A \, (1 + w^* A^{-1} v)$, which holds because the added matrix
has rank one. The dependence on $\alpha$ is affine; this identity is what replaces the
...
built along a fixed Hilbert basis. The two side conditions of the potential argument are
then removed as in \cref{ch:edge}, and the normalisation of the first family as in
\cref{thm:main}, so the theorem holds here in the same generality as for a finite second
family.
...
\section{The edge cases and the general form}

The two side conditions $m \geq 2$ and $M \geq 1 + 1/n$ of \cref{thm:main-infinite} are
removed exactly as in \cref{ch:edge}: where one of them fails, the corresponding potential
is replaced by a constant verifier of average $n$. On the upper side that constant is
$U(x) = n \|b(x)\|^2 / \Tr_e J$, and its average is $n$ because
$\int_\Omega \|b\|^2 \, d\mu = \Tr_e J$, which is \cref{lem:integral-inner-op} with
$Q = 1$. The lower side is the finite argument verbatim, since the first family is finite
in both cases, and the read-off needs only the operator form of \cref{lem:rank-one-crude}.

\begin{lemma}[Crude bound for a rank-one operator]
\label{lem:rank-one-crude-op}
\lean{ContinuousLinearMap.rankOne_le_norm_sq_smul_one}
\leanok
For every $u \in H$,
\[
  u u^* \; \preceq \; \|u\|^2 \cdot 1 .
\]
\end{lemma}

\begin{proof}
\leanok
A self-adjoint element $a$ of a C$^*$-algebra satisfies $a \preceq \|a\| \cdot 1$, and
$u u^*$ is self-adjoint with $\|u u^*\| = \|u\|^2$.
\end{proof}

\begin{theorem}[A one-element first family, with a countable second family]
\label{thm:card-one-infinite}
\lean{Discretization.Infinite.bss_generalized_of_unique}
\leanok
\uses{def:finite-trace-pos}
Let $\#\iota = 1$, let $a : \Omega \to \mathbb{C}^{\iota}$ be square-integrable with
$G(a) = 1$, let $b : \Omega \to H$ be square-integrable with Gram operator $J$ positive,
injective and of finite trace, let $\Lambda > 0$ with $J \preceq \Lambda \cdot 1$, and let
$n \geq 1$ satisfy $M = \Tr_e J / \Lambda \geq 1 + 1/n$. Then there are points
$x_1, \dots, x_n \in \Omega$ and weights $w_1, \dots, w_n > 0$ with
\[
  1 \; \preceq \; \sum_{i=1}^n w_i \, a(x_i) a(x_i)^*
  \qquad\text{and}\qquad
...
\begin{proof}
\leanok
\uses{thm:card-one, lem:barrier-op, lem:average-upper-op, lem:exists-point, lem:read-off}
The proof of \cref{thm:card-one}, with \cref{lem:barrier-op} and
\cref{lem:average-upper-op} on the upper side. The lower verifier is again the constant
$L(x) = n |a(x)|^2$, of average $n$, so the induction tracks the operator alone and the
largest admissible weight $1/w_i = L(x_i)$ makes the lower bound the identity
$\sum_i w_i |a(x_i)|^2 = 1$.
\end{proof}

\begin{theorem}[A small effective dimension, with a countable second family]
\label{thm:small-dim-infinite}
\lean{Discretization.Infinite.bss_generalized_of_small_dim}
\leanok
\uses{def:finite-trace-pos}
Let $a : \Omega \to \mathbb{C}^{\iota}$ be square-integrable with $G(a) = 1$, let
$b : \Omega \to H$ be square-integrable with Gram operator $J$ positive, injective and of
finite trace, let $\Lambda > 0$, let $m = \#\iota \geq 2$ and $n \geq m$, and assume
$M = \Tr_e J / \Lambda \leq 1 + 1/n$. Then there are points $x_1, \dots, x_n \in \Omega$
and weights $w_1, \dots, w_n > 0$ with
\[
  \Bigl(1 - \sqrt{\tfrac{m-1}{n}}\Bigr)^2 \cdot 1
    \; \preceq \; \sum_{i=1}^n w_i \, a(x_i) a(x_i)^*
\]
...
\begin{proof}
\leanok
\uses{thm:small-dim, lem:rank-one-crude-op, lem:integral-inner-op, lem:exists-point}
The induction of \cref{thm:small-dim} verbatim: it consumes the second family only through
the constant upper verifier $U$, here $U(x) = n \|b(x)\|^2 / \Tr_e J$, whose average is
$n$ by \cref{lem:integral-inner-op} with $Q = 1$. The weights it produces satisfy
$w_i \|b(x_i)\|^2 \leq \Tr_e J / n$, so \cref{lem:rank-one-crude-op} gives
$\sum_i w_i \, b(x_i) b(x_i)^* \preceq \Tr_e J \cdot 1 = M \Lambda \cdot 1$, and
$M \leq 1 + 1/n$ forces $M \leq (1 + \sqrt{(M-1)/n})^2$.
\end{proof}

\begin{theorem}[Both edge cases at once, with a countable second family]
\label{thm:both-edge-cases-infinite}
\lean{Discretization.Infinite.bss_generalized_of_unique_of_small_dim}
\leanok
\uses{def:finite-trace-pos}
Let $\#\iota = 1$, let $a : \Omega \to \mathbb{C}^{\iota}$ be square-integrable with
$G(a) = 1$, let $b : \Omega \to H$ be square-integrable with Gram operator $J$ positive,
injective and of finite trace, let $\Lambda > 0$ and $n \geq 1$, and assume
$M = \Tr_e J / \Lambda \leq 1 + 1/n$. Then there are points $x_1, \dots, x_n \in \Omega$
and weights $w_1, \dots, w_n > 0$ with
\[
  1 \; \preceq \; \sum_{i=1}^n w_i \, a(x_i) a(x_i)^*
...
\begin{proof}
\leanok
\uses{thm:both-edge-cases, lem:exists-point-le, lem:rank-one-crude-op,
      lem:integral-inner-op}
The proof of \cref{thm:both-edge-cases}. Both verifiers are the constants
$L(x) = n |a(x)|^2$ and $U(x) = n \|b(x)\|^2 / \Tr_e J$, of equal average $n$, so
\cref{lem:exists-point-le} produces a single point $y$ with $U(y) \leq L(y)$ and
$L(y) > 0$; using it $n$ times with the weight $w = 1/L(y)$ makes the lower bound the
identity, and $w \, U(y) \leq 1$ with \cref{lem:rank-one-crude-op} gives the upper bound.
No induction is needed.
\end{proof}

\begin{theorem}[Sparsification with a countable second family, general form]
\label{thm:main-infinite-general}
\lean{Discretization.Infinite.bss_generalized}
\leanok
\uses{def:gram, def:effective-dimension, def:finite-trace-pos}
Let $a : \Omega \to \mathbb{C}^{\iota}$ be square-integrable in every coordinate with
$I = G(a)$ positive definite, let $b : \Omega \to H$ be square-integrable with Gram
operator $J$ positive, injective and of finite trace, and let $\Lambda > 0$ satisfy
$J \preceq \Lambda \cdot 1$. Put $m = \#\iota$ and $M = \Tr_e J / \Lambda$. Then for every
$n \geq m$ there are points $x_1, \dots, x_n \in \Omega$ and weights
$w_1, \dots, w_n > 0$ with
\[
  \Bigl(1 - \sqrt{\tfrac{m-1}{n}}\Bigr)^2 \cdot I
    \; \preceq \; \sum_{i=1}^n w_i \, a(x_i) a(x_i)^*
...
\begin{proof}
\leanok
\uses{thm:main-infinite, thm:card-one-infinite, thm:small-dim-infinite,
      thm:both-edge-cases-infinite, lem:gram-mulvec}
A case distinction on $m \geq 2$ and on $M \geq 1 + 1/n$ reduces the case $I = 1$ to
\cref{thm:main-infinite} and to the three theorems above; for $m = 1$ the lower frame
constant $(1 - \sqrt{(m-1)/n})^2$ equals $1$, which is what those two state. That case is
\texttt{Discretization.Infinite.bss\_generalized\_of\_gram\_eq\_one'}. The normalisation is
then removed as in \cref{thm:main}: replace $a$ by $I^{-1/2} a$, whose Gram matrix is $1$
by \cref{lem:gram-mulvec}, and conjugate the resulting lower bound by $I^{1/2}$, which
preserves the Loewner order; the upper bound does not involve the first family and is
unchanged.
\end{proof}
...
\leanok
\uses{thm:main-infinite-general}
In the situation of \cref{thm:main-infinite-general} with $I = 1$, the same points and
weights satisfy, for every $c \in \mathbb{C}^{\iota}$,
\[
  \Bigl(1 - \sqrt{\tfrac{m-1}{n}}\Bigr)^2 \int_\Omega
...
\label{rem:left}
\leanok
The number of points in \cref{thm:main-infinite-general} is governed by the effective
dimension $M = \Tr_e J / \Lambda$ and not by the size of the second family. What is not formalised
...
\chapter{Discretizing the uniform norm}
\label{ch:design}

The sparsification theorem starts from a measure; this chapter produces one. Let $D$ be
an arbitrary set, $\iota$ a finite nonempty index set with $n = \#\iota$ elements, and
$a = (a_k)_{k \in \iota}$ a family of bounded functions $D \to \mathbb{C}$ whose
coordinate functions are linearly independent. Writing $f(y) = \langle c, a(y) \rangle$
for the function with coefficient vector $c \in \mathbb{C}^{\iota}$, we look for finitely
many points with weights for which
\[
  |f(y)|^2 \; \leq \; (n + \varepsilon) \sum_k w_k |f(x_k)|^2
  \qquad \text{for all } y \in D \text{ and all } c,
\]
that is, the uniform norm is dominated by the $L_2$ norm of that measure with the
constant $\sqrt{n + \varepsilon}$. The answer of Kiefer and Wolfowitz is that a measure
whose Gram matrix has an almost maximal determinant does this.

No topology and no measurable structure on $D$ are used: a finitely supported measure is
a finite list of points with weights, and it is kept in that form until
\cref{cor:design-measure}.

\begin{definition}[Design and its Gram matrix]
\label{def:design-gram}
\lean{Discretization.KieferWolfowitz.designGram}
\leanok
A \emph{design} consists of finitely many points $x_1, \dots, x_N \in D$ and weights
$w_1, \dots, w_N \geq 0$ with $\sum_k w_k = 1$. Its \emph{Gram matrix} is
\[
  G = \sum_{k=1}^N w_k \, a(x_k) a(x_k)^* \in \mathbb{C}^{\iota \times \iota},
\]
the Gram matrix of the family $a$ in $L_2$ of the measure $\sum_k w_k \delta_{x_k}$.
\end{definition}

\begin{lemma}[Quadratic form of a Gram matrix]
\label{lem:design-quadform}
\lean{Discretization.KieferWolfowitz.re_quadForm_designGram}
\leanok
For every design and every $c \in \mathbb{C}^{\iota}$,
$c^* G c = \sum_k w_k |\langle c, a(x_k) \rangle|^2$.
\end{lemma}

\begin{proof}
\leanok
The quadratic form of a rank-one matrix $u u^*$ at $c$ is $|\langle c, u \rangle|^2$,
and both sides are additive in the summands.
\end{proof}

\begin{lemma}[A design with an invertible Gram matrix]
\label{lem:nondegenerate}
\lean{Discretization.KieferWolfowitz.exists_design_posDef}
\leanok
If $c = 0$ is the only coefficient vector with $\langle c, a(y) \rangle = 0$ for all
$y \in D$, then some design has a positive definite Gram matrix.
\end{lemma}

\begin{proof}
\leanok
It suffices to find points annihilating no nonzero coefficient vector, for then the
uniform distribution on them has $c^* G c > 0$ for every $c \neq 0$ by
\cref{lem:design-quadform}. Descending induction on dimension supplies them: while some
nonzero vector survives the points chosen so far, the hypothesis gives a point at which
it does not vanish, and adding that point lowers the dimension.
\end{proof}

\begin{lemma}[Gain from mixing in one point]
\label{lem:mix}
\lean{Discretization.KieferWolfowitz.exists_mix_ge}
\leanok
Let $n \geq 1$ and $t > n$ be real. Then $\alpha = (t-n) / (2n(t-1))$ lies in $[0,1)$ and
\[
  1 + \frac{(t-n)^2}{4n(t-1)}
    \; \leq \; (1-\alpha)^{n-1} \bigl( 1 + \alpha (t-1) \bigr).
\]
\end{lemma}

\begin{proof}
\leanok
By Bernoulli's inequality $(1-\alpha)^{n-1} \geq 1 - (n-1)\alpha$, and the difference of
the two sides of the claim is then the explicit nonnegative quantity
$(t-n)^2 / (4n^2(t-1))$. The weight $\alpha$ is the one for which the linear gain
$\alpha(t-n)$ is twice the quadratic loss $(n-1)(t-1)\alpha^2$, so half of the gain
survives.
\end{proof}

\begin{proposition}[Almost maximal determinant]
\label{prop:det-max}
\lean{Discretization.KieferWolfowitz.exists_design_quadForm_inv_le}
\leanok
\uses{def:design-gram, lem:nondegenerate, lem:det-rank-one, lem:mix}
Assume the functions $a_k$ are bounded and their coordinate functions linearly
independent, and let $\varepsilon > 0$. Then there is a design whose Gram matrix $G$ is
positive definite and satisfies
\[
  a(y)^* G^{-1} a(y) \; \leq \; n + \varepsilon \qquad \text{for all } y \in D .
\]
\end{proposition}

\begin{proof}
\leanok
The determinants of the Gram matrices of designs form a set of real numbers which is
bounded above, since the entries of every Gram matrix are bounded by $C^2$ and the
Leibniz formula bounds a determinant by its entries, and which contains a positive
number by \cref{lem:nondegenerate}. Let $S$ be its supremum and take a design whose
determinant exceeds $S / (1 + \kappa)$, where
$\kappa = \varepsilon^2 / (4n(n + \varepsilon - 1))$; its Gram matrix $G$ is positive
definite because its determinant is positive.

Appending a point $y$ with weight $\alpha \in [0,1)$ and scaling the old weights by
$1 - \alpha$ is again a design, with Gram matrix $(1-\alpha) G + \alpha \, a(y) a(y)^*$.
By \cref{lem:det-rank-one} its determinant is $\det G$ multiplied by
$(1-\alpha)^{n-1}(1 + \alpha(t-1))$ with $t = a(y)^* G^{-1} a(y)$, and that multiple
cannot exceed $S / \det G < 1 + \kappa$. If $t$ were larger than $n + \varepsilon$, the
weight of \cref{lem:mix} would make it at least $1 + \kappa$.
\end{proof}

\begin{theorem}[Discretization of the uniform norm]
\label{thm:uniform-discretization}
\lean{Discretization.KieferWolfowitz.exists_design_kieferWolfowitz}
\leanok
\uses{prop:det-max, lem:design-quadform, lem:update-posdef}
Let $a$ be bounded with linearly independent coordinate functions and let
$\varepsilon > 0$. Then there are points $x_1, \dots, x_N \in D$ and weights
$w_1, \dots, w_N \geq 0$ with $\sum_k w_k = 1$ such that
\[
  \Bigl| \sum_k \overline{c_k} \, a_k(y) \Bigr|^2
    \; \leq \; (n + \varepsilon) \sum_{k=1}^N w_k
      \Bigl| \sum_l \overline{c_l} \, a_l(x_k) \Bigr|^2
\]
for every $y \in D$ and every $c \in \mathbb{C}^{\iota}$.
\end{theorem}

\begin{proof}
\leanok
Take the design of \cref{prop:det-max} for $\varepsilon/2$. For $s = n + \varepsilon$
the bound $s^{-1} a(y)^* G^{-1} a(y) < 1$ is strict, so subtracting $s^{-1} a(y) a(y)^*$
from $G$ leaves a positive definite matrix by \cref{lem:update-posdef}, that is
$a(y) a(y)^* \preceq s \cdot G$. Evaluating this at $c$ and using
\cref{lem:design-quadform} gives the claim.
\end{proof}

\begin{theorem}[The sharp constant on a compact domain]
\label{thm:uniform-discretization-compact}
\lean{Discretization.KieferWolfowitz.exists_design_kieferWolfowitz_of_compact}
\leanok
\uses{prop:det-max, lem:mix, lem:design-quadform}
Let $D$ be a compact space and let the functions $a_k$ be continuous with linearly
independent coordinate functions. Then there are points $x_1, \dots, x_N \in D$ and
weights $w_1, \dots, w_N \geq 0$ with $\sum_k w_k = 1$ such that
\[
  \Bigl| \sum_k \overline{c_k} \, a_k(y) \Bigr|^2
    \; \leq \; n \sum_{k=1}^N w_k \Bigl| \sum_l \overline{c_l} \, a_l(x_k) \Bigr|^2
\]
for every $y \in D$ and every $c \in \mathbb{C}^{\iota}$.
\end{theorem}
...
\begin{proof}
\leanok
The matrices $a(y) a(y)^*$ form a compact set, whose convex hull is compact as well and
consists exactly of the Gram matrices of designs. The determinant is continuous, so it
attains its maximum at a design. No mixture can then increase the determinant at all,
and \cref{lem:mix} applied with the factor $1$ gives $a(y)^* G^{-1} a(y) \leq n$ without
slack. The passage to the norm inequality is that of
\cref{thm:uniform-discretization} for every constant $s > n$, followed by the limit
$s \to n$, which is a limit of real numbers.
\end{proof}

Both theorems produce a list of points with weights. Read as a measure, that list is
the object the sparsification theorem asks for.

\begin{corollary}[The measure]
\label{cor:design-measure}
\lean{Discretization.KieferWolfowitz.exists_probabilityMeasure_kieferWolfowitz}
\leanok
\uses{thm:uniform-discretization, def:gram}
Let $D$ carry a measurable structure, let the functions $a_k$ be bounded and measurable
with linearly independent coordinate functions, and let $\varepsilon > 0$. Then the
points and weights of \cref{thm:uniform-discretization} give a probability measure
$\varrho = \sum_k w_k \delta_{x_k}$ whose Gram matrix $G(a)$ in the sense of
\cref{def:gram} is positive definite and equal to the Gram matrix of the design, and
\[
  |f(y)|^2 \; \leq \; (n + \varepsilon) \int_D |f|^2 \, d\varrho
\]
for every $y \in D$ and every $f$ in the span of the family.
\end{corollary}

\begin{proof}
\leanok
Integration against $\sum_k w_k \delta_{x_k}$ is the weighted sum over the points, for
every strongly measurable integrand.
\end{proof}

\begin{lemma}[Few points suffice]
\label{lem:point-count}
\lean{Discretization.KieferWolfowitz.exists_design_card_le}
\leanok
\uses{def:design-gram}
Every design can be replaced by one with at most $2n^2 + 1$ points and the same Gram
matrix.
\end{lemma}

\begin{proof}
\leanok
The Gram matrix lies in the convex hull of the matrices $a(y) a(y)^*$, so by
Carath\'eodory's theorem it is a convex combination of affinely independent ones, and an
affinely independent family in the real vector space $\mathbb{C}^{\iota \times \iota}$,
whose real dimension is $2n^2$, has at most $2n^2 + 1$ members.
\end{proof}

\begin{remark}[What the count is good for]
\label{rem:point-count}
\leanok
\uses{lem:point-count, thm:uniform-discretization}
Both \cref{thm:uniform-discretization} and \cref{thm:uniform-discretization-compact}
depend on the design only through its Gram matrix, by \cref{lem:design-quadform}, so
\cref{lem:point-count} gives them with at most $2n^2 + 1$ points. The sharper count
$r + 1$, with $r$ the dimension of the span of the products $a_k \overline{a_l}$, is not
formalised, and nothing needs it.
\end{remark}

\begin{remark}[The identity decomposed over the design]
\label{rem:identity-decomposition}
\lean{Discretization.KieferWolfowitz.sum_weight_quadForm_inv_eq_card, Discretization.KieferWolfowitz.sum_weight_smul_quadForm_inv_eq_self}
\leanok
\uses{def:design-gram, prop:det-max, thm:uniform-discretization-compact}
For every design whose Gram matrix $G$ is invertible,
\[
  \sum_{k=1}^N w_k \, a(x_k)^* G^{-1} a(x_k) = n
  \qquad \text{and} \qquad
  \sum_{k=1}^N w_k \bigl( a(x_k)^* G^{-1} z \bigr) a(x_k) = z
\]
for every $z \in \mathbb{C}^{\iota}$, the first being $\Tr(G^{-1} G)$ and the second
$G G^{-1} z = z$ written out. With $c_k = w_k \, a(x_k)^* G^{-1} a(x_k)$ this is John's
decomposition of the identity over the design points, in the inner product
$\langle u, z \rangle = u^* G^{-1} z$ of the ellipsoid of $G$, and it is the condition
dual to the bound of \cref{prop:det-max}. Under
\cref{thm:uniform-discretization-compact} the average forces
$a(x_k)^* G^{-1} a(x_k) = n$ at every design point, which makes the points contact
points. For an arbitrary convex body the theorem is proved in the companion project
\href{https://mario-ullrich.github.io/Lean-SNumbers/}{Lean-SNumbers}
(\href{https://mario-ullrich.github.io/Lean-SNumbers/blueprint.pdf}{PDF}).
\end{remark}
...
SIAM Review \textbf{56} (2014), no.~2, 315--334,
\href{https://doi.org/10.1137/130949117}{doi},
\href{https://arxiv.org/abs/0808.0163}{arxiv}.
...
spaces},
preprint, 2026,
\href{https://arxiv.org/abs/2602.18719}{arxiv}.

\bibitem{KW}
J.~Kiefer, J.~Wolfowitz,
\emph{The equivalence of two extremum problems},
Canad. J. Math. \textbf{12} (1960), 363--366,
\href{https://doi.org/10.4153/CJM-1960-030-4}{doi}.

\bibitem{KPUU}
D.~Krieg, K.~Pozharska, M.~Ullrich, T.~Ullrich,
\emph{Sampling projections in the uniform norm},
J. Math. Anal. Appl. \textbf{553} (2026), no.~2, Paper No.~129873,
\href{https://arxiv.org/abs/2401.02220}{arxiv}.
